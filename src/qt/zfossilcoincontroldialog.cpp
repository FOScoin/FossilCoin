...
#include "ui_zfossilcoincontroldialog.h"
...
std::list<std::string> ZFossilCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFossilCoinControlDialog::listMints;

ZFossilCoinControlDialog::ZFossilCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFossilCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZFossilCoinControlDialog::~ZFossilCoinControlDialog()
{
    delete ui;
}

void ZFossilCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFossilCoinControlDialog::updateList()
{
...
void ZFossilCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFossilCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFossilCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFossilCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFossilCoinControlDialog::GetSelectedMints()
{
...
void ZFossilCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
