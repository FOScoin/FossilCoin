...
#include "zfossilcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zFOS ought to be enough for anybody." - Bill Gates, 2017
    ui->zFOSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFOSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzFOS->setEnabled(false);
        ui->pushButtonMintzFOS->setToolTip(tr("zFOS is currently disabled due to maintenance."));

        ui->pushButtonSpendzFOS->setEnabled(false);
        ui->pushButtonSpendzFOS->setToolTip(tr("zFOS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFOSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFOS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFOS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFOS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFOS in ") +
...
void PrivacyDialog::on_pushButtonSpendzFOS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFOS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zFOS
        sendzFOS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFOS
    sendzFOS();
}

void PrivacyDialog::on_pushButtonZFossilCoinControl_clicked()
{
    ZFossilCoinControlDialog* zFossilCoinControl = new ZFossilCoinControlDialog(this);
    zFossilCoinControl->setModel(walletModel);
    zFossilCoinControl->exec();
}

void PrivacyDialog::setZFossilCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFossilCoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFOS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid FossilCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFOSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFOS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FOS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFOS</b>";
    ...
    // use mints from zFossilCoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFossilCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZFossilCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfossilcoin selector in case it was used
    ZFossilCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFossilCoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " FossilCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFossilCoin Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFOS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFOS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFOS "));
    ui->labelzFOSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzFOSSyncStatus->setVisible(fShow);
}
