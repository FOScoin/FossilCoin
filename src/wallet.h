...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 FOS at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zFOS send
enum ZerocoinSpendStatus {
    ZFOS_SPEND_OKAY = 0,                            // No error
    ZFOS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFOS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFOS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFOS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFOS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFOS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFOS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFOS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFOS_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFOS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFOS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFOS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFOS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFOS_SPENT_USED_ZFOS = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZFossilCoinAutoBackups(bool fEnabled)
    {
